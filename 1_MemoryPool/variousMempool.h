...
namespace variousMemoryPool
{
	...
	public:
		cPoolArr_queue(int poolSize, bool placementNew = true);
		~cPoolArr_queue();
	...
	private:
		stDataNode<DATA>* _pHead;			// 리스트 구조 메모리풀 시작부 포인터
		stDataNode<DATA>* _pTail;			// 리스트 구조 메모리풀 끝부분 포인터
		stDataNode<DATA>* _pStart;			// 메모리 해제를 위한 시작부 포인터
	...
	public:
		cPoolList_queue(int poolSize, bool placementNew = true);
		~cPoolList_queue();
	...
	template <typename DATA>
	cPoolArr_queue<DATA>::cPoolArr_queue(int poolSize, bool placementNew)
	{
		_totalSize = poolSize;
		_usedSize = 0;
		_bPlacementNew = placementNew;
	...
	template <typename DATA>
	cPoolArr_queue<DATA>::~cPoolArr_queue()
	{
		if (!_bPlacementNew)
		{
			for (int i = 0; i < _totalSize; ++i)
	...
	template <typename DATA>
	DATA* cPoolArr_queue<DATA>::Alloc()
	{
		if (nullptr == _pHead)
		{
			return nullptr;
		...
		AcquireSRWLockExclusive(&_srwPool);
		stDataNode<DATA>* pRetNode = _pHead;
	...
	template <typename DATA>
	void cPoolArr_queue<DATA>::Free(DATA* pFree)
	{
		if (nullptr != pFree)
		{
			UINT_PTR position = (UINT_PTR)pFree;
			...
			if (nullptr == _pTail)
			{
				_pHead = _pTail = pFreeNode;
				pFreeNode->_pNext = nullptr;
			}
			else
			{
				//stDataNode<DATA>* pTmp = _pTail;
				//pTmp->_pNext = pFreeNode;
				//pFreeNode->_pNext = nullptr;
				//_pTail = pFreeNode;
				_pTail->_pNext = pFreeNode;
				pFreeNode->_pNext = nullptr;
				_pTail = pFreeNode;
			}
			--_usedSize;
	...
	template <typename DATA>
	cPoolList_queue<DATA>::cPoolList_queue(int poolSize, bool placementNew)
	{
		_totalSize = poolSize;
		_usedSize = 0;
		_bPlacementNew = placementNew;
	...
	template <typename DATA>
	cPoolList_queue<DATA>::~cPoolList_queue()
	{
		while (_pHead)
		{
			stDataNode<DATA>* pNextNode = _pHead->_pNext;
			if (!_bPlacementNew)
			{
				_pHead->_data.~DATA();
			}
	...
	template <typename DATA>
	DATA* cPoolList_queue<DATA>::Alloc()
	{
		// 초기에 지정한 크기를 넘어서는 경우
		if (nullptr == _pHead)
		{
	...
	template <typename DATA>
	void cPoolList_queue<DATA>::Free(DATA* pFree)
	{
		if (nullptr != pFree)
		{
			UINT_PTR position = (UINT_PTR)pFree;
