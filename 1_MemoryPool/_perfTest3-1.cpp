...
// 테스트 3-2와의 차이점은 캐시 라인 안에 두 개의 객체가 들어갈 수 없는 크기
...
unsigned WINAPI Test_SingleThread(LPVOID lpParam)
{
	...
	vector<stTest128*> addrVec(ALLOC_COUNT * 4);

	if (0 == paramArr[0])
	{
		cPoolArr_stack<stTest128>* pAlloc = (cPoolArr_stack<stTest128>*)paramArr[1];
		for (int loopCnt = 0; loopCnt < WARMUP_LOOP; ++loopCnt)
		{
			stTest128* pNew = pAlloc->Alloc();
			pAlloc->Free(pNew);
		}
		for (int loopCnt = 0; loopCnt < LOGIC_LOOP; ++loopCnt)
		{
			...
			for (int allocCnt = 0; allocCnt < ALLOC_COUNT * 4; ++allocCnt)
			{
				...
				addrVec[allocCnt]->_size[64] = (char)0xaa;
			}
			for (int allocCnt = 0; allocCnt < ALLOC_COUNT * 4; ++allocCnt)
			{
	...
	{
		cPoolList_stack<stTest128>* pAlloc = (cPoolList_stack<stTest128>*)paramArr[1];
		for (int loopCnt = 0; loopCnt < WARMUP_LOOP; ++loopCnt)
		{
			stTest128* pNew = pAlloc->Alloc();
			pAlloc->Free(pNew);
...
{
	LPVOID* paramArr = (LPVOID*)lpParam;
	vector<stTest128*> addrVec(ALLOC_COUNT);

	if (0 == paramArr[0])
	{
		cPoolArr_stack<stTest128>* pAlloc = (cPoolArr_stack<stTest128>*)paramArr[2];
		for (int loopCnt = 0; loopCnt < WARMUP_LOOP; ++loopCnt)
		{
			stTest128* pNew = pAlloc->Alloc();
			pAlloc->Free(pNew);
	...
	{
		cPoolList_stack<stTest128>* pAlloc = (cPoolList_stack<stTest128>*)paramArr[2];
		for (int loopCnt = 0; loopCnt < WARMUP_LOOP; ++loopCnt)
		{
			stTest128* pNew = pAlloc->Alloc();
			pAlloc->Free(pNew);
...
int main()
{
	...
	g_hFile = CreateFile(L"Test3-1_Result.txt", GENERIC_READ | GENERIC_WRITE, NULL, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
	...
	//{
	//	cPoolArr_stack<stTest128> arrStack(ALLOC_COUNT * 4);
	...
	//	wss << L"\n*** [array stack - Single Thread 128 bytes] ***\n";
	...
	//{
	//	cPoolList_stack<stTest128> listStack(ALLOC_COUNT * 4);
	...
	//	wss << L"\n*** [list stack - Single Thread 128 bytes] ***\n";
	//	WriteFile(g_hFile, wss.str().c_str(), (DWORD)wss.str().size() * 2, NULL, NULL); wss.str(L"");
	//	wss << L"[평균 소요 시간: " << to_wstring(g_singleThreadResult) << L" us]\n";
	//	WriteFile(g_hFile, wss.str().c_str(), (DWORD)wss.str().size() * 2, NULL, NULL); wss.str(L"");
	//	CloseHandle(hThread);
	...
	wprintf(L"스택형 배열 128 bytes 할당 작업 시작 - 멀티 스레드\n");
	{
		...
		wss << L"\n*** [array stack - Multi Thread 128 bytes] ***\n";
		WriteFile(g_hFile, wss.str().c_str(), (DWORD)wss.str().size() * 2, NULL, NULL); wss.str(L"");
	...
	wprintf(L"스택형 리스트 128 bytes 할당 작업 시작 - 멀티 스레드\n");
	{
		hJobReadyEvent1 = CreateEvent(NULL, FALSE, FALSE, NULL);
		hJobReadyEvent2 = CreateEvent(NULL, FALSE, FALSE, NULL);
		hJobReadyEvent3 = CreateEvent(NULL, FALSE, FALSE, NULL);
		...
		wss << L"\n*** [list stack - Multi Thread 128 bytes] ***\n";
		WriteFile(g_hFile, wss.str().c_str(), (DWORD)wss.str().size() * 2, NULL, NULL); wss.str(L"");
		wss << L"[평균 소요 시간: " << to_wstring(g_multiThreadResult) << L" us]\n";
		WriteFile(g_hFile, wss.str().c_str(), (DWORD)wss.str().size() * 2, NULL, NULL); wss.str(L"");
		for (int i = 0; i < 4; ++i)
